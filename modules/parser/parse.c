...
static void ICACHE_FLASH_ATTR readColor(char dataBuf[7], BOOL dry,  struct color_t *color) {
	...
	uint32_t r_val = (uint32_t)((r > 255 ? 255 : r < 0 ? 0 : r) * MAX_PERIOD / 255);
	uint32_t g_val = (uint32_t)((g > 255 ? 255 : g < 0 ? 0 : g) * MAX_PERIOD / 255);
	uint32_t b_val = (uint32_t)((b > 255 ? 255 : b < 0 ? 0 : b) * MAX_PERIOD / 255);
