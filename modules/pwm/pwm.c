...
static void pwm_intr_handler(void) {
	...
	do {
		...
		gpio->out_w1ts =
				(uint32_t) (pwm_state.current_set[pwm_state.current_phase].on_mask);
		gpio->out_w1tc =
				(uint32_t) (pwm_state.current_set[pwm_state.current_phase].off_mask);
...
_pwm_phases_prep(struct pwm_phase* pwm) {
	...
	for (n = 0; n < phases; n++) {
		pwm[n].ticks = pwm[n + 1].ticks - pwm[n].ticks;
...
pwm_start(void) {
	...
	if ((*pwm == pwm_state.next_set) || (*pwm == pwm_state.current_set))
		pwm++;
	if ((*pwm == pwm_state.next_set) || (*pwm == pwm_state.current_set))
		pwm++;
